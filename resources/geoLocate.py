...
# @file    geoLocate.py (tileGet.py + extension for georeferencing traffic network files)
# @author  Michael Behrisch + Halvor Groven
# @date    2021-04-29
...
optParser.add_option("-z", "--originlat", default="-1", help="latitude for origin (ex. 60.5938324")
optParser.add_option("-y", "--originlon", default="-1", help="longditute for origin (ex. 10.42323")
...
def get(args=None):
    ...
    if not options.net:
        optParser.error("Please specify net-file.")
        return
    if not options.originlat:
        optParser.error("Please set required parameter --olon")
        return
    if not options.originlon:
        optParser.error("Please set required parameter --olat")
        return

    def convertlatlong2XY(lat,lng, proj_string):
        projection = Proj(proj_string)
        return projection(lat,lng)

    tuple2 = convertlatlong2XY(float(options.originlat), float(options.originlon),"+proj=utm +zone=32 +ellps=WGS84 +datum=WGS84 +units=m +no_defs")

    with open (options.net, 'r' ) as f:
        content = f.read()
        print(tuple2[0])
        content_new = re.sub('netOffset=".+?(?=").', 'netOffset="-'+str(tuple2[0])+',-' + str(tuple2[1])+ '"', content, flags = re.M)
        content_newest = re.sub('projParameter=".+?(?=").', 'projParameter="+proj=utm +zone=32 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"', content_new, flags = re.M)
        print(content_new)
    f.close()
    with open (options.net, 'w' ) as f:
        f.write(content_newest)
